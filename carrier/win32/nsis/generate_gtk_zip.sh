...
if [ ! -e $PIDGIN_BASE/ChangeLog ]; then
	echo $(basename $0) must must have the carrier base dir specified as a parameter.
	exit 1
fi

STAGE_DIR=$PIDGIN_BASE/carrier/win32/nsis/gtk_runtime_stage
...
do
	...
	if [ ! -e $PIDGIN_BASE/po/$LOCALE.po ]; then
		echo Removing $LOCALE translation as it is missing from Carrier
		rm -r $LOCALE_DIR
	fi
done
