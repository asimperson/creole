...
!ifdef WITH_GTK
OutFile "carrier-${PIDGIN_VERSION}.exe"
!else
!ifdef DEBUG
OutFile "carrier-${PIDGIN_VERSION}-debug.exe"
!else
OutFile "carrier-${PIDGIN_VERSION}-no-gtk.exe"
!endif
!endif
...
!define PIDGIN_UNINSTALL_KEY			"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Carrier"

!define HKLM_APP_PATHS_KEY			"SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\carrier.exe"
!define STARTUP_RUN_KEY				"SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
!define PIDGIN_UNINST_EXE			"carrier-uninst.exe"
...
VIAddVersionKey "ProductName" "Carrier"
VIAddVersionKey "FileVersion" "${PIDGIN_VERSION}"
VIAddVersionKey "ProductVersion" "${PIDGIN_VERSION}"
VIAddVersionKey "LegalCopyright" ""
!ifdef WITH_GTK
VIAddVersionKey "FileDescription" "Carrier Installer (w/ GTK+ Installer)"
!else
!ifdef DEBUG
VIAddVersionKey "FileDescription" "Carrier Installer (Debug Version)"
!else
VIAddVersionKey "FileDescription" "Carrier Installer (w/o GTK+ Installer)"
!endif
!endif
...
;Modern UI Configuration
  ...
  !define MUI_ICON				".\pixmaps\carrier-install.ico"
  !define MUI_UNICON				".\pixmaps\carrier-install.ico"
  !define MUI_WELCOMEFINISHPAGE_BITMAP		".\pixmaps\carrier-intro.bmp"
  !define MUI_HEADERIMAGE
  !define MUI_HEADERIMAGE_BITMAP		".\pixmaps\carrier-header.bmp"
  ...
  !define MUI_FINISHPAGE_RUN			"$INSTDIR\carrier.exe"
  !define MUI_FINISHPAGE_RUN_NOTCHECKED
  !define MUI_FINISHPAGE_LINK			$(PIDGIN_FINISH_VISIT_WEB_SITE)
  !define MUI_FINISHPAGE_LINK_LOCATION	"http://funpidgin.sourceforge.net"
...
Section $(PIDGIN_SECTION_TITLE) SecPidgin
  ...
  carrier_hkcu:
    ...
    WriteRegStr HKCU "${PIDGIN_UNINSTALL_KEY}" "DisplayName" "Carrier"
...
SectionGroup /e $(PIDGIN_SHORTCUTS_SECTION_TITLE) SecShortcuts
  Section /o $(PIDGIN_DESKTOP_SHORTCUT_SECTION_TITLE) SecDesktopShortcut
    SetOverwrite on
    CreateShortCut "$DESKTOP\Carrier.lnk" "$INSTDIR\carrier.exe"
    SetOverwrite off
  SectionEnd
  Section $(PIDGIN_STARTMENU_SHORTCUT_SECTION_TITLE) SecStartMenuShortcut
    SetOverwrite on
    CreateShortCut "$SMPROGRAMS\Carrier.lnk" "$INSTDIR\carrier.exe"
    SetOverwrite off
  SectionEnd
SectionGroupEnd
...
Function RegisterURIHandler
  ...
  WriteRegStr HKCR "$R0\DefaultIcon" "" "$INSTDIR\carrier.exe"
  WriteRegStr HKCR "$R0\shell" "" ""
  WriteRegStr HKCR "$R0\shell\Open" "" ""
  WriteRegStr HKCR "$R0\shell\Open\command" "" "$INSTDIR\carrier.exe --protocolhandler=%1"
  Pop $R0
FunctionEnd
...
Function .onInit
  ...
  IfErrors done_preselecting_shortcuts
    ...
    GetFileTime "$DESKTOP\Carrier.lnk" $R0 $R0
    IfErrors +1 +5
    ClearErrors
    SetShellVarContext "all"
    GetFileTime "$DESKTOP\Carrier.lnk" $R0 $R0
  ...
  preselect_startmenu_shortcut:
    ...
    GetFileTime "$SMPROGRAMS\Carrier.lnk" $R0 $R0
  ...
  StrCmp $R0 "HKLM" 0 user_dir
    StrCpy $INSTDIR "$PROGRAMFILES\Carrier"
    Goto instdir_done
  user_dir:
    Push $SMPROGRAMS
    ${GetParent} $SMPROGRAMS $R2
    ${GetParent} $R2 $R2
    StrCpy $INSTDIR "$R2\Carrier"
