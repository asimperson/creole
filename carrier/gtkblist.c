...
#include "gtkutils.h"
#include "carrier/minidialog.h"
#include "carrier/pidgintooltip.h"
...
gchar *funpidgin_get_group_title(PurpleBlistNode *gnode);
...
pidgin_blist_get_status_icon(PurpleBlistNode *node, PidginStatusIconSize size)
{
	...
	if (purple_prefs_get_bool(PIDGIN_PREFS_ROOT "/blist/funpidgin_protocol_as_status")) {
		if (((icon == PIDGIN_STOCK_STATUS_AVAILABLE)||(icon == PIDGIN_STOCK_STATUS_AVAILABLE_I)) && (purple_prefs_get_bool(PIDGIN_PREFS_ROOT "/blist/funpidgin_hide_available"))) {
			ret = pidgin_create_prpl_icon(account, PIDGIN_PRPL_ICON_SMALL);
		} else {
			ret = funpidgin_overlay(pidgin_create_prpl_icon(account, PIDGIN_PRPL_ICON_SMALL), gtk_widget_render_icon (GTK_WIDGET(gtkblist->treeview), icon,
...
static gboolean insert_node(PurpleBuddyList *list, PurpleBlistNode *node, GtkTreeIter *iter)
{
	...
	if (node->parent) {
		if (!get_iter_from_node(node->parent, &parent_iter)) {
			return FALSE;
		}
	}
...
gchar *funpidgin_get_group_title(PurpleBlistNode *gnode)
{
	...
	title = pidgin_get_group_title(gnode, TRUE);
	first_set = g_strsplit(title, "<b>", 2);
	second_set = g_strsplit(first_set[1], "</b>", 2);
