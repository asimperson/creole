...
funpidgin_page(void)
{
	...
	/* GtkWidget *group_box, *delimiter_box; */
	GtkWidget *check_box;
	GtkWidget *ind_box;
	GtkWidget *dep_box;
	GtkWidget *contrast;
...
#if 0					 
	group_box = pidgin_prefs_checkbox(_("_Emulate subgroup behaviour for delimited group names"),
				PIDGIN_PREFS_ROOT "/blist/funpidgin_subgroups", vbox);
	
	delimiter_box = pidgin_prefs_labeled_entry(vbox, _("Delimiter string:"), 
				PIDGIN_PREFS_ROOT "/blist/funpidgin_subgroup_delimiter", sg);
	
	if (!(purple_prefs_get_bool(PIDGIN_PREFS_ROOT "/blist/funpidgin_subgroups"))) {
		gtk_widget_set_sensitive(delimiter_box, FALSE);
	}
	g_signal_connect(G_OBJECT(group_box), "clicked",
					 G_CALLBACK(pidgin_toggle_sensitive), delimiter_box);
	g_signal_connect(G_OBJECT(group_box), "clicked",
					 G_CALLBACK(funpidgin_refresh_blist_cb), delimiter_box);
#endif
